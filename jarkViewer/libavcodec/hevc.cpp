...
static int pic_arrays_init(HEVCContext *s, const HEVCSPS *sps)
{
    ...
    s->sao           = (SAOParams*)av_mallocz_array(ctb_count, sizeof(*s->sao));
    s->deblock       = (DBParams*)av_mallocz_array(ctb_count, sizeof(*s->deblock));
    if (!s->sao || !s->deblock)
        goto fail;

    s->skip_flag    = (uint8_t*)av_malloc(sps->min_cb_height * sps->min_cb_width);
    s->tab_ct_depth = (uint8_t*)av_malloc_array(sps->min_cb_height, sps->min_cb_width);
    if (!s->skip_flag || !s->tab_ct_depth)
        goto fail;

    s->cbf_luma = (uint8_t*)av_malloc_array(sps->min_tb_width, sps->min_tb_height);
    s->tab_ipm  = (uint8_t*)av_mallocz(min_pu_size);
    s->is_pcm   = (uint8_t*)av_malloc((sps->min_pu_width + 1) * (sps->min_pu_height + 1));
    if (!s->tab_ipm || !s->cbf_luma || !s->is_pcm)
        goto fail;

    s->filter_slice_edges = (uint8_t*)av_malloc(ctb_count);
    s->tab_slice_address  = (int32_t*)av_malloc_array(pic_size_in_ctb,
    ...
    s->horizontal_bs = (uint8_t*)av_mallocz_array(s->bs_width, s->bs_height);
    s->vertical_bs   = (uint8_t*)av_mallocz_array(s->bs_width, s->bs_height);
...
static int set_sps(HEVCContext *s, const HEVCSPS *sps)
{
    ...
    if (sps->vui.colour_description_present_flag) {
        s->avctx->color_primaries = (AVColorPrimaries)sps->vui.colour_primaries;
        s->avctx->color_trc       = (AVColorTransferCharacteristic)sps->vui.transfer_characteristic;
        s->avctx->colorspace      = (AVColorSpace)sps->vui.matrix_coeffs;
    } else {
    ...
    if (sps->sao_enabled) {
#ifdef USE_SAO_SMALL_BUFFER
        {
            ...
            s->sao_pixel_buffer = 
                (uint8_t*)av_malloc(((ctb_size + 2) * (ctb_size + 2)) <<
                          s->sps->pixel_shift);
            for(c_idx = 0; c_idx < c_count; c_idx++) {
                int w = s->sps->width >> s->sps->hshift[c_idx];
                int h = s->sps->height >> s->sps->vshift[c_idx];
                s->sao_pixel_buffer_h[c_idx] = 
                    (uint8_t*)av_malloc((w * 2 * s->sps->ctb_height) <<
                              s->sps->pixel_shift);
                s->sao_pixel_buffer_v[c_idx] = 
                    (uint8_t*)av_malloc((h * 2 * s->sps->ctb_width) <<
                              s->sps->pixel_shift);
            }
        }
#else
...
static int hls_slice_header(HEVCContext *s)
{
    ...
    if (!sh->dependent_slice_segment_flag) {
        ...
        sh->slice_type = (SliceType)get_ue_golomb_long(gb);
    ...
    if (s->pps->tiles_enabled_flag || s->pps->entropy_coding_sync_enabled_flag) {
        ...
        if (sh->num_entry_point_offsets > 0) {
            ...
            av_freep(&sh->offset);
            av_freep(&sh->size);
            sh->entry_point_offset = (int*)av_malloc_array(sh->num_entry_point_offsets, sizeof(int));
            sh->offset = (int*)av_malloc_array(sh->num_entry_point_offsets, sizeof(int));
            sh->size = (int*)av_malloc_array(sh->num_entry_point_offsets, sizeof(int));
                              ...
                              int cbf_luma, int *cbf_cb, int *cbf_cr)
{
    ...
    if (cbf_luma || cbf_cb[0] || cbf_cr[0] ||
        ...
        if (cbf_luma)
            ff_hevc_hls_residual_coding(s, x0, y0, log2_trafo_size, (ScanType)scan_idx, 0);
        if (s->sps->chroma_format_idc != 0) {
        if (log2_trafo_size > 2 || s->sps->chroma_format_idc == 3) {
            ...
            for (i = 0; i < (s->sps->chroma_format_idc == 2 ? 2 : 1); i++) {
                ...
                if (cbf_cb[i])
                    ff_hevc_hls_residual_coding(s, x0, y0 + (i << log2_trafo_size_c),
                                                log2_trafo_size_c, (ScanType)scan_idx_c, 1);
                else
                    if (lc->tu.cross_pf) {
            ...
            for (i = 0; i < (s->sps->chroma_format_idc == 2 ? 2 : 1); i++) {
                ...
                if (cbf_cr[i])
                    ff_hevc_hls_residual_coding(s, x0, y0 + (i << log2_trafo_size_c),
                                                log2_trafo_size_c, (ScanType)scan_idx_c, 2);
                else
                    if (lc->tu.cross_pf) {
            ...
            for (i = 0; i < (s->sps->chroma_format_idc == 2 ? 2 : 1); i++) {
                ...
                if (cbf_cb[i])
                    ff_hevc_hls_residual_coding(s, xBase, yBase + (i << log2_trafo_size),
                                                log2_trafo_size, (ScanType)scan_idx_c, 1);
            }
            for (i = 0; i < (s->sps->chroma_format_idc == 2 ? 2 : 1); i++) {
                if (lc->cu.pred_mode == MODE_INTRA) {
                    ff_hevc_set_neighbour_available(s, xBase, yBase + (i << log2_trafo_size),
                ...
                if (cbf_cr[i])
                    ff_hevc_hls_residual_coding(s, xBase, yBase + (i << log2_trafo_size),
                                                log2_trafo_size, (ScanType)scan_idx_c, 2);
            }
        }
        } /* chroma_firmat_idc != 0 */
...
static int hls_coding_unit(HEVCContext *s, int x0, int y0, int log2_cb_size)
{
...
#endif
    {
        ...
        if (lc->cu.pred_mode != MODE_INTRA ||
            ...
            lc->cu.part_mode        = (PartMode)ff_hevc_part_mode_decode(s, log2_cb_size);
...
{
    HEVCContext *s  = (HEVCContext*)avctxt->priv_data;
    int ctb_size    = 1 << s->sps->log2_ctb_size;
    int more_data   = 1;
    int x_ctb       = 0;
    int y_ctb       = 0;
...
static int hls_nal_unit(HEVCContext *s)
{
    ...
    s->nal_unit_type = (NALUnitType)get_bits(gb, 6);
...
static int hevc_frame_start(HEVCContext *s)
{
    ...
    s->frame->pict_type = (AVPictureType)(3 - s->sh.slice_type);
...
static int decode_nal_units(HEVCContext *s, const uint8_t *buf, int length)
{
    ...
    while (length >= 4) {
        ...
        if (s->nals_allocated < s->nb_nals + 1) {
            ...
            HEVCNAL *tmp = (HEVCNAL*)av_realloc_array(s->nals, new_size, sizeof(*tmp));
            if (!tmp) {
                ret = AVERROR(ENOMEM);
                goto fail;
            }
            ...
            s->skipped_bytes_pos_nal[s->nals_allocated] = (int*)av_malloc_array(s->skipped_bytes_pos_size_nal[s->nals_allocated], sizeof(*s->skipped_bytes_pos));
                             ...
                             AVPacket *avpkt)
{
    ...
    if (!avpkt->size) {
        ret = ff_hevc_output_frame(s, (AVFrame*)data, 1);
        if (ret < 0)
            return ret;
...
#endif
        av_frame_move_ref((AVFrame*)data, s->output_frame);
        *got_output = 1;
    }
...
static av_cold int hevc_init_context(AVCodecContext *avctx)
{
    ...
    s->HEVClc = (HEVCLocalContext*)av_mallocz(sizeof(HEVCLocalContext));
    if (!s->HEVClc)
        goto fail;
    s->HEVClcList[0] = s->HEVClc;
    s->sList[0] = s;

    s->cabac_state = (uint8_t*)av_malloc(HEVC_CONTEXTS);
    if (!s->cabac_state)
        goto fail;
