...
const unordered_set<wstring> supportExt = {
    ...
    L".exr", L".tiff", L".tif", L".webp", L".hdr", L".pic" , L".heic" , L".heif" };
...
static int myMain(const std::wstring& _fullPath, HINSTANCE hInstance) {
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
    ...
    wstring fullPath(*lpCmdLine == '\0' ? L"" : (*lpCmdLine == '\"' ? lpCmdLine + 1 : lpCmdLine));
    if (fullPath.length() && fullPath[fullPath.length() - 1] == L'\"')
        fullPath.pop_back();
