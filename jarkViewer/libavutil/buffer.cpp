...
int av_buffer_realloc(AVBufferRef **pbuf, int size)
{
    ...
    if (!(buf->buffer->flags & BUFFER_FLAG_REALLOCATABLE) ||
        ...
        AVBufferRef *tmpPtr = NULL;

        av_buffer_realloc(&tmpPtr, size);
        if (!tmpPtr)
            return AVERROR(ENOMEM);

        memcpy(tmpPtr->data, buf->data, FFMIN(size, buf->size));
...
{
    AVBufferPool *pool = (AVBufferPool*)av_mallocz(sizeof(*pool));
    if (!pool)
        return NULL;
...
{
    BufferPoolEntry *cur = (BufferPoolEntry*)(*(void * volatile *)&pool->pool), *last = NULL;
...
static AVBufferRef *pool_alloc_buffer(AVBufferPool *pool)
{
    ...
    buf = (BufferPoolEntry*)av_mallocz(sizeof(*buf));
    if (!buf) {
        av_buffer_unref(&ret);
        return NULL;
    }
